...
export (PackedScene) var blaster_shot
...
func shoot():
	if can_shoot and ammo_count > 0:
		ammo_count -=1
		can_shoot = false
		var blast = blaster_shot.instance()
		parent.get_parent().add_child(blast)
		blast.global_transform = $muzzle.global_transform
		blast.velocity = blast.transform.basis.z * blast.speed
